...
to go
  ...
  if lift? and (ticks mod (days-between-workouts - days-deviation) = 0)
    [ lift-weights ]
  sleep
  regulate-hormones
  develop-muscle
  set muscle-mass sum [fiber-size] of muscle-fibers
  apply-temporary-deviation
  age-rate
  tick
end
...
to perform-daily-activity
  ...
  ask patches [
    set anabolic-hormone anabolic-hormone + (log food 10) * 5
    set catabolic-hormone catabolic-hormone + (log food 10) * 4
  ]
end
...
to lift-weights
  ...
  ask muscle-fibers [
    if( random-float 1.0 < (intensity - intensity-deviation) / 100 * (intensity - intensity-deviation) / 100 ) [
...
to sleep
  ...
  ask patches [
    set catabolic-hormone catabolic-hormone - 0.5 * (log catabolic-hormone 10) * (hours-of-sleep - sleep-deviation)
    set anabolic-hormone anabolic-hormone - 0.48 * (log anabolic-hormone 10) * (hours-of-sleep - sleep-deviation)
  ]
end
